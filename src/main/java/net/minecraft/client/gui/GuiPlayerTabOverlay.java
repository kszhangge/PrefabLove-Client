...
public class GuiPlayerTabOverlay extends Gui {
    ...
    public String getPlayerName(NetworkPlayerInfo networkPlayerInfoIn) {
        ...
        for (Member member : Chat.getInstance().getOnlineMembers().values()) {
            if (networkPlayerInfoIn.getGameProfile().getName().equals(member.getMcName())) {
                name = String.format(TextFormatting.GREEN + "[%s" + TextFormatting.GREEN + "] %s%s", member.getName(), member.isFriend() ? (TextFormatting.GREEN + "(Friend) ") : "", TextFormatting.RESET + name);
                break;
            }
        }
        ...
        if (PrefabLove.moduleManager.getModuleByClass(HackerDetector.class).isEnabled()) {
            PlayerData playerData = ((HackerDetector) PrefabLove.moduleManager.getModuleByClass(HackerDetector.class)).getPlayData(networkPlayerInfoIn.getGameProfile().getId());
