...
public class FileManager {
    public static final File CLIENT_DIRECTORY = new File(mc.gameDir, PrefabLove.CLIENT_NAME);
    public static final File CLIENT_INFO = new File(CLIENT_DIRECTORY, PrefabLove.CLIENT_NAME + ".json");

    public FileManager() {
        if (!CLIENT_DIRECTORY.exists()) {
            if (!CLIENT_DIRECTORY.mkdir()) {
                PrefabLove.LOGGER.error("Failed to create the client file folder!");
            }
        }
    ...
    public void updateMetadata(JsonObject jsonObject) {
        ...
        try {
            ...
            metadataJsonObject.addProperty("version", PrefabLove.CLIENT_VERSION);
