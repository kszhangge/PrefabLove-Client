        ...
        while (!mc.preProcessedTasks.isEmpty()) {
            ...
            if (mc.getConnection() != null && mc.world != null && mc.player != null && task instanceof PacketProcessListenableFutureTask<?> packetTask) {
                if (!forceSkipAbilities && flyTicks <= 180 && packetTask.packetToProcess instanceof SPacketPlayerAbilities abilities
                        ...
                        && (mc.player.capabilities.isFlying || PrefabLove.moduleManager.getModuleByClass(Flight.class).isEnabled())
                        && PrefabLove.moduleManager.getModuleByClass(SpectatorAbuse.class).isEnabled()
...
//                }
        ...
        if (PrefabLove.moduleManager != null) {
            Flight flight = (Flight) PrefabLove.moduleManager.getModuleByClass(Flight.class);
            if (flyTicks == 1) {
    ...
    public static boolean shouldProcess() {
        if (PrefabLove.moduleManager == null) return false;
