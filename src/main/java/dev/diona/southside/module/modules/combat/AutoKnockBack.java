...
public class AutoKnockBack extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        if (!delay.hasReached(delayValue.getValue().doubleValue()) || Blink.isInstanceEnabled()
                || !((KillAura) PrefabLove.moduleManager.getModuleByClass(KillAura.class)).targets.isEmpty()
                || RotationUtil.targetRotation != null
                || PrefabLove.moduleManager.getModuleByClass(Scaffold.class).isEnabled()
                || mc.player.isHandActive() || (RotationUtil.targetRotation != null && ((KillAura) PrefabLove.moduleManager.getModuleByClass(KillAura.class)).target == null)
        )
            return;
    ...
    public void onMotion(MotionEvent event) {
        if (Blink.isInstanceEnabled() || PrefabLove.moduleManager.getModuleByClass(Scaffold.class).isEnabled() &&
                PrefabLove.moduleManager.getModuleByClass(AutoGapple.class).isEnabled()) return;
        if (event.getState() == EventState.POST) {
            if (autoSwitchValue.getValue()) {
                ...
                } else if (RotationUtil.targetRotation != null && target != null && ((KillAura) PrefabLove.moduleManager.getModuleByClass(KillAura.class)).targets.isEmpty() && mc.player.inventory.getCurrentItem().getItem() != Items.BOW) {
