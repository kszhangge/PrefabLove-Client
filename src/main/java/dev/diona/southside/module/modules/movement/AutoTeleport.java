...
//    public BooleanValue shiftValue = new Switch("Shift TP", true);
    ...
    public boolean onEnable() {
        ...
        if (mc.player.capabilities.isFlying) {
            alinking = true;
            PrefabLove.moduleManager.getModuleByClass(Alink.class).setEnable(true);
            PrefabLove.moduleManager.getModuleByClass(Flight.class).setEnable(true);
            this.move(0, 5, 0);
        } else {
    ...
    public boolean onDisable() {
        if (Alink.isInstanceEnabled()) {
            PrefabLove.moduleManager.getModuleByClass(Flight.class).setEnable(false);
            return false;
        }
        return true;
    }
...
//        }
        if (alinking && !Flight.isInstanceEnabled()) {
            alinking = false;
            PrefabLove.moduleManager.getModuleByClass(Alink.class).setEnable(false);
            this.setEnable(false);
        }
    }
