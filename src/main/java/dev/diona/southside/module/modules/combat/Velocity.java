...
public class Velocity extends Module {
    ...
    public void onPacket(PacketEvent event) {
        ...
        if (event.getPacket() instanceof SPacketEntityVelocity packet) {
            ...
            if (("Grim".equals(mode) || "Mix".equals(mode)) && !PrefabLove.moduleManager.getModuleByClass(Scaffold.class).isEnabled()) {
                velocity = false;
...
//                    RotationUtil.setTargetRotation(new Rotation(fixedYaw, mc.player.rotationPitch), 2);
        ...
        if ("Grim".equals(mode) || "Mix".equals(mode)) {
            ...
            if (velocity && !attacked) {
                ...
                if (/* !Blink.isInstanceEnabled() && */ result != null && result.typeOfHit == RayTraceResult.Type.ENTITY && Target.isTarget(result.entityHit) /* &&  result.entityHit == KillAura.getTarget() */) {
                ...
                if (entity == null || !entity.isEntityAlive()) {
                    PrefabLove.eventBus.post(new FailReduceKBEvent());
                    return;
                }
...
//                    }
                ...
                PrefabLove.eventBus.post(new AttackEvent(entity, EventState.PRE));
                PrefabLove.eventBus.post(new AttackEvent(entity, EventState.POST));
                int count = grimLegitValue.getValue() ? 1 : 6;
                for (int i = 1; i <= count; i++) {
                    if (PrefabLove.moduleManager.getModuleByClass(AutoGapple.class).isEnabled()) {
                        final AutoGapple autoGapple = (AutoGapple) PrefabLove.moduleManager.getModuleByClass(AutoGapple.class);
                ...
                ((AutoGapple) PrefabLove.moduleManager.getModuleByClass(AutoGapple.class)).velocityed = true;
//                    if (blocking) {
//                        NoSlow.grimPost();
//                    }
            }
