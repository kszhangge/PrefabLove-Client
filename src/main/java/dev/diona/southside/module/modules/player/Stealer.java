...
//        InvManager invManager = (InvManager) PrefabLove.moduleManager.getModuleByClass(InvManager.class);
//        if (!invManager.timer.hasReached(auraDelay.getValue().doubleValue())) return;
//        if (aura.getValue() && !PrefabLove.moduleManager.getModuleByClass(Blink.class).isEnabled()) {
//            if (mc.player.openContainer.windowId != 0) {
//                stealTimer.reset();
//                return;
//            }
//            if (/*PrefabLove.moduleManager.getModuleByClass(Scaffold.class).isEnabled() || */PrefabLove.moduleManager.getModuleByClass(Scaffold.class).isEnabled() || PrefabLove.moduleManager.getModuleByClass(Blink.class).isEnabled())
//                return;
...
//        }
    ...
    public final void onUpdate(final UpdateEvent event) {
        ...
        if (mc.player.openContainer.windowId == 0) { // 没有打开任何外部容器
             ...
             * Chest Aura
             */
            if (aura.getValue() && mc.currentScreen == null && !PrefabLove.moduleManager.getModuleByClass(Blink.class).isEnabled()) {
                final var tile = mc.world.loadedTileEntityList.stream()
            ...
            if (mc.player.openContainer instanceof ContainerChest chest) {
                ...
                for (int i : slots) {
                    if (exploitValue.getValue()) {
                        ...
                        if (test == -1) {
                            List<Dropdown> slotValue = ((InvManager) PrefabLove.moduleManager.getModuleByClass(InvManager.class)).slotValue;
...
//        return INSTANCE.modeValue.getValue().equals("Silent");
    ...
    private List<Integer> addUsefulItem(List<Integer> slots) {
        ...
        AutoWeapon autoWeapon = ((AutoWeapon) PrefabLove.moduleManager.getModuleByName("AutoWeapon"));
        for (int i : slots) {
